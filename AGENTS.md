...
- **Gate-week batch 1 landed (2026-07-10):** streaming fixed (deltas keyed by inner message id — one growing bubble); usage panel real (OAuth usage-endpoint poller per account + DB boot-hydrate; header meter deleted, left-rail caps panel is the one home); **agent system v1** — global pool (all agents `scope:'global'`), 6 stock specialists seeded (authoritative drift-reseed) + orchestrator seeded insert-only, `/api/agents/pods` CRUD + attach/detach, Agents-tab edit surface, project-settings Agents section. Orchestrator system prompt/model live from its agents row; edits apply next message (rev-based backend re-mint with SDK resume).
...
- **Known gaps (by design):** activity panel empty until Phase 3 run endpoints; agents are definitions-only (dispatch = Phase 3; agent tools/effort stored but inert); usage poller rides an unofficial OAuth endpoint (fallback = passive rate-limit events + persisted snapshots); expired account token → that account's usage stays last-known until a turn refreshes it; no periodic MCP re-probe; MCP-attachment + secrets editing per agent deferred (slots into http/agents.ts).
