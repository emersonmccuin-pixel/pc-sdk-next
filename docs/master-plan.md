...
- *Content:* `Markdown`, `MermaidBlock`, chat bubbles, diff views, attachments + lightbox, `SubscriptionQuotaPanel` (seeds future provider-aware usage views; the full dashboard remains N6).
...
`c146162` after concurrent UI-only landings. The evidence rejects the inherited
project-only anchor and proposes optional
generic PM context plus an exact external item reference, with AInativePM owning
